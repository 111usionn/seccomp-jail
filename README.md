...
- [seccomp-jail 项目文档](#seccomp-jail-项目文档)
  ...
  - [演示视频](#演示视频)
  - [目标分析](#目标分析)
    - [1. 能够根据固定的规则对直接/间接启动的进程的系统调用进行过滤](#1-能够根据固定的规则对直接间接启动的进程的系统调用进行过滤)
    - [2. 能够动态地对直接/间接启动的进程的系统调用进行过滤， 指当系统调用发生以后弹出窗口展示程序的系统参数， 根据用户的选择对系统调用进行放行；](#2-能够动态地对直接间接启动的进程的系统调用进行过滤-指当系统调用发生以后弹出窗口展示程序的系统参数-根据用户的选择对系统调用进行放行)
    - [3. 能够动态地根据用户输入生成对应系统调用的结果。](#3-能够动态地根据用户输入生成对应系统调用的结果)
    - [总结](#总结)
  - [原理介绍](#原理介绍)
    - [PTRACE](#ptrace)
    - [SECCOMP](#seccomp)
...
2024.7.30 添加对返回值的处理,补充了相关文档和测试 
...
video：[度盘](https://pan.baidu.com/s/1wNNbqxOxRVpplGv-RsHLvg?pwd=rf6w)

## 目标分析

### 1. 能够根据固定的规则对直接/间接启动的进程的系统调用进行过滤

即seccomp可以实现的基本功能，加载好过滤器后进行fork，过滤器会一同被复制到子进程中，再由子进程执行exec函数，用目标程序替换该子进程，这个过程中过滤器同样会继续生效，至此，即达成了该目标。

### 2. 能够动态地对直接/间接启动的进程的系统调用进行过滤， 指当系统调用发生以后弹出窗口展示程序的系统参数， 根据用户的选择对系统调用进行放行；

seccomp存在一些限制，例如一旦过滤器加载，则本次运行中无法再卸载；在同一次运行中，可以不断添加新的规则，但无法删除旧的规则等等。这使得我们必须在被监控程序运行前了解该程序需要使用哪些系统调用，并编写适当的规则加以限制，显示出了seccomp灵活性的不足。
为了解决灵活性的问题，linux内核在5.0版本引入了seccomp-unotify机制。被监控程序称为target，接受消息并决定target行为的程序称为supervisor。该机制可以在target的系统调用发生后阻塞target，并把系统调用该如何执行的决定权转交给supervisor动态判断，至此，即实现了该目标。(https://man7.org/linux/man-pages/man2/seccomp_unotify.2.html)

seccomp也可以在拦截到系统调用后生成ptrace事件，再由tracer捕获该事件，也可以达成相同的效果。

### 3. 能够动态地根据用户输入生成对应系统调用的结果。

可分为两种模式。

(1)在某个受到监控的系统调用发生后，拦截该系统调用，并修改为想要主动执行的系统调用，然后放行。

(2)在程序运行过程中的任意时刻，打断程序执行，插入系统调用并执行，然后恢复程序正常执行。

(1)的实现如果在只使用seccomp的情况下，修改参数只需要在resp结构体中修改并发回即可。修改内存则需要读写/proc/[pid]/mem。如果使用seccomp+ptrace，直接使用ptrace提供的功能就可以完成修改。两种方案都可以比较简单地解决。

(2)的实现如果在只使用seccomp的情况下，没有较简单修改程序代码段、寄存器的方式，但是使用ptrace可以相对简单地操作进程的启停和读写寄存器/内存以完成该功能。

### 总结

| 方案 | seccomp | seccomp + ptrace |
|:-------|:-------|:-------|
| 代码量 | 少 | 多 |
| 功能 | 少 | 多 |
| 效率 | 高 | 低 |
...
一开始只使用了seccomp，在编码遇到瓶颈后，换用seccomp+ptrace尝试。最终在对比两种方案后最终采用了seccomp+ptrace的方案。

一个问题：为什么不只使用ptrace？只靠ptrace自己就可以捕获系统调用。

主要还是效率问题，ptrace在设置PTRACE_SYSCALL后，会在下一个任意系统调用的开始/结束时暂停进程，因而会拦截所有的系统调用。而两者结合使用可以先用seccomp过滤掉大部分不需要考虑的系统调用，再由ptrace处理剩余的，减少内核态和用户态之间反复切换的次数。
...
在远程监控时使用，处在被监控机上，负责通过socket接受和转发各种指令。
...
下载打包好的Release版本并解压，将AppImage可执行文件和libhookhere.so放在同一目录,运行AppImage即可使用；如需自行编译源码运行，且环境中原已有Fluentui控件库，请注意编译此项目时会导致其中一些控件的行为被改变。libhookhere的源文件在源码目录内，自行编译为so后即可使用（只是一个空的动态库，为了存放主动添加的指令）
...
规则文件是一json文件，存储各系统调用及其规则。规则有入口规则和出口规则两种。

一个系统调用对应一个Json Array，内有四个元素，arr[0]~arr[3]分别为入口规则号，脚本的base64编码(可能为空),出口规则号，要改为的返回值(可能为空)。
...
设置上面两条永久规则主要是为了提高运行效率，永久规则不会经过用户态,适合一些规则已经确定的系统调用或程序运行必不可少，不需要监控的系统调用。以下的规则可以在程序运行过程中动态切换。
...
系统根据用户自定义的bash脚本判断执行规则为Notify、Allow或Abort，在系统调用捕获后，会把发起调用的进程pid，六个参数，六个解引用后的参数分别存储在变量：PID、SJ_ARG1-SJ_ARG6、SJ_DARG1-SJ_DARG6中，可供在脚本中使用。(解引用的字长默认为1个字即64位数据，等设置页面做好后可以在里面调整，现除修改源码外暂无方式调整)
...
下图是一个例子，意为判断该系统调用第一个参数是否为123，若是则交给用户处理，否则直接通过。
...
提示到前端，让用户选择是否更改返回值。(返回值显示在arg1处，更改也在此处。更改其他栏无效果)

##### 例子

如图所示的是83号(mkdir)系统调用的规则，含义为：当mkdir发生时，执行脚本并根据结果判断实际是何种规则，当mkdir在出口时，更改其返回值为 -233。

![image](https://github.com/user-attachments/assets/bb1ee880-0d6f-4a34-b4f9-8706b1822b6e)
...
先在远程主机上运行seccomp-jail-net-core并指定要监听的端口：
```
$ ./seccomp-jail-net-core 2333
```
在本地主机，点击Remote转到远程模式，用
```
ip:port path
```
的格式确定远程主机上要监控的程序，后续操作相同。

#### 监测系统调用

除Notify规则外，其他规则自动审批，查看日志即可;设置了Notify规则的系统调用会显示详细信息在主界面等待用户处理，具体细节如下。

##### 系统调用入口

进程pid，系统调用号/名，参数等信息会显示在界面上。双击对应的栏位，可修改对应的参数（系统调用号和六个参数）。直到用户选择一种行动前，请求该系统调用的进程处于暂停状态。可选择的行动有五种：
...
3. 如果被监控程序需要使用标准输入输出，请在终端中运行seccomp-jail，当被监控程序启动后，即可使用该终端窗口进行标准输入输出（故远程监控时无法在此进行输入输出），后续可能添加单独的界面处理标准输入输出。

4. 如果只需要监控系统调用的返回值，仍不能将入口规则设为两个forever规则，因为程序不会为这两个规则生成事件。
...
更改getpid系统调用的返回值，使程序运行到正常运行流程中不可能达到的区域
...
| 系统调用名 | 入口规则 | 出口规则 | 用户操作 | 备注 |
|:-------|:-------|:-------|:-------|:-------|
| getpid | allow | notify | 把返回值改为 -23333 |  |
| 其他 | allow forever | default |  |  |
...
读文档时发现了seccomp的unotify机制，该机制可以把seccomp拦截到的系统调用转发至其他进程处理，并且在执行fork、exec等操作时，seccomp的过滤器会被内核自动拷贝进新进程，且结合libseccomp提供的易用性很高的api，能够十分方便得实现基本的进程动态监控功能。
所以在编写初版程序时，便采用了单seccomp的方式，并没有使用ptrace。但在后续开发过程中发现，由于seccomp只在系统调用入口暂停进程，所以无法读取到系统调用的返回值，且还有无法主动执行系统调用等等问题
...
异常处理还不完善（如用户输入了不存在的路径、远程主机意外断开等情况）。
